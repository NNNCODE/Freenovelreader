...
//    private var asyncTask: AsyncTask<Int, Void, MutableList<TxtPage>>? = null
    ...
    private fun drawContent(bitmap: Bitmap) {
        ...
        if (mStatus != STATUS_FINISH) {
            ...
            var tip = ""
            when (mStatus) {
                STATUS_LOADING -> tip = "The first loading is relatively slow, please be patient and wait"
                STATUS_ERROR -> tip = "Loading failed (click on edge to retry)"
                STATUS_EMPTY -> tip = "The content of the article is empty"
                STATUS_PARING -> tip = "Typing in progress, please wait"
                STATUS_PARSE_ERROR -> tip = "File parsing error"
                STATUS_CATEGORY_EMPTY -> tip = "The directory list is empty"
            }
