    ...
    override fun initListener() {
        ...
        tv_search_title.setOnClickListener {
            MaterialDialog(requireContext()).show {
                title(text = "Book City Classification")
        ...
        tv_search_filter.setOnClickListener {
            MaterialDialog(requireContext()).show {
                title(text = "Novel classification")
        ...
        tv_search_search.setOnClickListener {
            MaterialDialog(requireContext()).show {
                title(text = "Search for novels")
                input(
                    hint = "Enter book title and author",
                    ...
                    if (keyword.isNotBlank()) {
                        ...
                        viewModel.toastMsg("Keywords cannot be empty")
                    }
                }
        ...
        viewModel.bookInserted.observe(viewLifecycleOwner, {
            viewModel.toastMsg("Successfully added to bookshelf")
        })
        ...
        viewModel.bookInfo.observe(viewLifecycleOwner, {
            if (it?.bookBean != null) {
                ...
                MaterialDialog(requireContext()).show {
                    ...
                    //icon(drawable = image)
                    message(text = "Author：${bookBean.author}\nType：${bookBean.category}\nState：${bookBean.status}\nIntroduction：${bookBean.desc}")
                    positiveButton(text = "start reading") {
                        val bookBean = viewModel.getBookBean()
                        if (bookBean != null) {
                            // 打开 书籍
                            NovelReadActivity.startFromActivity(requireActivity(), bookBean)
                        } else {
                            viewModel.toastMsg("Opening the book is abnormal, please retrieve the book again")
                        }
                    }
                    negativeButton(text = "Join the bookshelf") {
                        //加入书架
